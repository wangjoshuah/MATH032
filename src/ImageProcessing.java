...
public class ImageProcessing {
	...
	/**
	 * Inverts the colors of every pixel (subtract from 255)
	 * Ex: if a color was white (255, 255, 255) it would become black (0, 0, 0)
	 */
	private void inverse() {
		// Nested loop over every pixel
		for (int y = 0; y < current.getHeight(); y ++) { //loop through the Y values
			for (int x = 0; x < current.getWidth(); x ++) { //for each Y, loop through all X's
				// Get the color, then subtract it from 255
				Color color = new Color(current.getRGB(x,y));
				int red = 255 - color.getRed(); //we don't need to worry about min/max (constrain) here because
				int green = 255 - color.getGreen(); //the original pixel value cannot be greater than 255 or less than 0
				int blue = 255 - color.getBlue(); //so the result when you subtract it from 255 will still be within those bounds
				// Put new color
				Color newColor = new Color(red, green, blue);
				current.setRGB(x, y, newColor.getRGB());
			}
		}
		System.out.println("Image Inverted"); //check that I finished through the for loops
	}
	...
	public static void main(String[] args) { 
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				ImageProcessing proc = new ImageProcessing(filePath);
			}
		});
	}
}
